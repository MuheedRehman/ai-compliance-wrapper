...
1. Module 6 Control Management lifecycle operations are implemented locally and verified; this is the next customer-readiness deploy target.
...
- Latest deployed staging baseline: commit `1876f6d` via Cloud Build `ff197d4a-42e7-4906-8756-e1057e9ae94d` with status `SUCCESS`.
- Live staging URLs:
...
- Latest verified Cloud Run revisions:
  - Dashboard: `ai-compliance-dashboard-00047-thx`
  - Backend: `ai-compliance-backend-00091-vkw`
...
| 6. Control Management | In progress | Compliance controls, readiness scorecard, evidence coverage, evidence attachment, owner/status/due date editing, severity, review cadence, review history, and review comments are implemented locally and verified. | Deploy the lifecycle operations slice, then add reusable control templates and exportable audit status. |
...
- Module 6 control lifecycle operations completed locally and release-verified: control responses now expose severity, evidence-required/evidence-complete, review cadence, last/next review timestamps, review-overdue status, review history, and latest comments from existing control metadata; `/v1/compliance/controls/{control_id}/reviews` records review notes/history and can update status/severity/cadence; the Controls dashboard can edit owner/status/due date/severity/review cadence and record review notes inline. Local verification passed: `tests/test_compliance_controls.py`, full backend `tests`, dashboard lint, and dashboard build.
...
- [ ] Deploy Module 6 control lifecycle operations to GCP staging and update the deployed baseline.
