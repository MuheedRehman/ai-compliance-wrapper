...
1. Module 5 Evidence Vault artifact preview workflow is deployed; next evidence hardening can add an external object-storage backend.
...
- Module 5 Evidence Vault Artifact Previews: deployed to staging. Uploaded evidence artifacts now have a read-authenticated inline preview endpoint for safe text, image, JSON/XML/CSV/Markdown, and PDF content; unsupported binary types remain download-only. The Evidence Vault dashboard can open previewable uploaded files in an audit preview panel with file metadata and hash context.
